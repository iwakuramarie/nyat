...
import (
	...
	"gitea.com/iwakuramarie/nyat/completer"
...
func NewComposer(nyat *Nyat, acct *AccountView, conf *config.NyatConfig,
	...
	email, err := ioutil.TempFile("", "nyat-compose-*.eml")
	if err != nil {
	...
	c.buildComposeHeader(nyat, cmpl)
...
func (c *Composer) buildComposeHeader(nyat *Nyat, cmpl *completer.Completer) {

	c.layout = nyat.conf.Compose.HeaderLayout
	...
	for i, row := range c.layout {
		for j, h := range row {
			...
			e := newHeaderEditor(h, c.header, nyat.SelectedAccount().UiConfig())
			if nyat.conf.Ui.CompletionPopovers {
				e.input.TabComplete(cmpl.ForHeader(h),
					nyat.SelectedAccount().UiConfig().CompletionDelay)
			}
			c.editors[h] = e
			switch h {
			case "from":
	...
	for _, h := range []string{"cc", "bcc"} {
		if c.header.Has(h) {
			if _, ok := c.editors[h]; !ok {
				e := newHeaderEditor(h, c.header, nyat.SelectedAccount().UiConfig())
				if nyat.conf.Ui.CompletionPopovers {
					e.input.TabComplete(cmpl.ForHeader(h), nyat.SelectedAccount().UiConfig().CompletionDelay)
				}
				c.editors[h] = e
				c.focusable = append(c.focusable, e)
...
func (c *Composer) AddEditor(header string, value string, appendHeader bool) {
	...
	if e, ok := c.editors[header]; ok {
		...
		e := newHeaderEditor(header, c.header,
			c.nyat.SelectedAccount().UiConfig())
