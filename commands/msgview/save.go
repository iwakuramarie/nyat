...
func (Save) Complete(nyat *widgets.Nyat, args []string) []string {
	path := strings.Join(args, " ")
	return commands.CompletePath(path)
}

func (Save) Execute(nyat *widgets.Nyat, args []string) error {
	...
	defaultPath := nyat.Config().General.DefaultSavePath
	...
	mv, ok := nyat.SelectedTab().(*widgets.MessageViewer)
	if !ok {
	...
	go func() {
		err := <-ch
		if err != nil {
			nyat.PushError(fmt.Sprintf("Save failed: %v", err))
			return
		}
		nyat.PushStatus("Saved to "+path, 10*time.Second)
	}()
	return nil
}
...
func generateFilename(part *models.BodyStructure) string {
	...
	if fn, ok := part.DispositionParams["filename"]; ok {
		...
		filename = fmt.Sprintf("nyat_%v", timestamp)
	}
	return filename
}
