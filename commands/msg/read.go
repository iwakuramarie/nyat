...
func (FlagMsg) Complete(nyat *widgets.Nyat, args []string) []string {
	return nil
}
...
func (FlagMsg) Execute(nyat *widgets.Nyat, args []string) error {
	...
	if len(toEnable) != 0 {
		submitFlagChange(nyat, store, toEnable, flag, true, &wg, &success)
	}
	if len(toDisable) != 0 {
		submitFlagChange(nyat, store, toDisable, flag, false, &wg, &success)
	}
	...
	go func() {
		wg.Wait()
		if success {
			nyat.PushStatus(actionName+" flag '"+flagName+"' successful", 10*time.Second)
		}
	}()
...
func submitFlagChange(nyat *widgets.Nyat, store *lib.MessageStore,
