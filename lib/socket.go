...
type NyatServer struct {
	logger   *log.Logger
	listener net.Listener
	OnMailto func(addr *url.URL) error
}

func StartServer(logger *log.Logger) (*NyatServer, error) {
	sockpath := path.Join(xdg.RuntimeDir(), "nyat.sock")
...
func (as *NyatServer) Close() {
	as.listener.Close()
}

var lastId int64 = 0 // access via atomic

func (as *NyatServer) handleClient(conn net.Conn) {
